...
| version | `cargo run -q -p atlas -- --version` | prints 0.1.6 |
...
| mcp-version | `cd line && go run ./cmd/atlas-mcp --version` | 0.1.6 |
...
| tui-version | `cd line && go run ./cmd/atlas-tui --version` | atlas-tui 0.1.6 |
...
| version | Response contains 0.1.6 | version matches |
...
```
push/PR → prove.yml
  ├── the battery (windows)      every version pin in sync; then
  │                              tests/prove.py, the whole thing (the fast
  │                              legs only, no cargo, until 2026-09-22)
  └── the go half, off Windows   go build + go test, line AND webapp
                                 gofmt -l gated across both modules

tag v*  → release.yml
  └── cut (windows)              the tag must equal VERSION; every pin in sync;
